...
class SimCollection:
    ...
    def write_simulators(self, stream, debug=0, individual=False, test_label='default'):
        ## Emit source macros
        ...
        for simname in simnames:
            ...
            stream.write('\n')
            sim.write_simulator(stream, 0, individual, test_label)

    def write_unit_tests(self, stream, debug=0, individual=False, test_label='default'):
        dontexpand = set([smac for smac, uses in self.macro_uses.items() if len(uses) > 1])
        ...
        for simname in simnames:
            ...
            ## in the macro sources:
            sim.sources = self.expand_sources(sim.sources, dontexpand, debug)
            sim.write_unit_test(stream, 0, individual, test_label)
