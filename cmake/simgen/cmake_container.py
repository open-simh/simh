        ...
        for dir in dirnames:
            ...
            if individual:
                ...
                with open(subdir_cmake, "w") as stream2:
                    ...
                    stream2.write('endif ()')
                    stream2.write('\n')
                    simcoll.write_simulators(stream2, debug, individual, dir)

                ## Write out unit tests, if the unit test subdirectory exists:
                subdir_units_dir = os.path.join(toplevel_dir, dir, 'unit-tests')
                ...
                if os.path.exists(subdir_units):
                    with open(subdir_units, "w") as stream2:
                        ...
                        stream2.write('\n')
                        simcoll.write_unit_tests(stream2, debug, individual, dir)
            else:
                stream.write('\n## ' + '-' * 40 + '\n')
                simcoll.write_simulators(stream, debug, individual, dir)
