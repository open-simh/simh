...
bool sbdrop(struct sbuf *sb, size_t num)
{
    ...
    if (sizeof(num) > sizeof(int) && num > UINT32_MAX)
        g_critical("sbdrop: sizeof(num) > sizeof(int), num = %" SLIRP_PRIsize_t "\n", num);
...
{
    if (sizeof(size) > sizeof(int) && size > UINT32_MAX)
        g_critical("sbreserve: sizeof(size) > sizeof(int), size = %" SLIRP_PRIsize_t "\n", size);
...
void sbappend(struct socket *so, struct mbuf *m)
{
    ...
    if (!so->so_rcv.sb_cc) {
        ...
        /* Almost impossible for this g_critical() to execute, except in a OS or hardware
         * error. */
        if (sizeof(n_sent) > sizeof(int) && n_sent > (slirp_ssize_t) m->m_len)
            g_critical("sbappend: n_sent > m->m_len, n_sent = %" SLIRP_PRIssize_t "\n", n_sent);
...
void sbcopy(struct sbuf *sb, size_t off, size_t len, char *to)
{
     ...
     *
     * g_critical() allows us to output more info even when debugging is not
     * enabled. */
    if (ptr_diff == 0 && (sb->sb_cc != 0 || sb->sb_cc != sb->sb_datalen)) {
        g_critical("sbcopy: ptr_diff == 0: sb->sb_cc (%" PRIu32 ") != sb->sb_datalen (%" PRIu32 "), sb->sb_cc != 0\n",
